...
var pst = [4, 1];
var pcl = 100;
var background = "en_icons/background.png"
...
icon("en_icons/start.png", "start");
icon("en_icons/end.png", "end");
icon("en_icons/int.png", "int");
icon("en_icons/double.png", "double");
icon("en_icons/char.png", "char");
icon("en_icons/print.png", "print");
icon("en_icons/scan.png", "scan");
icon("en_icons/if_1.png", "if_1");
icon("en_icons/if_2.png", "if_2");
icon("en_icons/up.png", "up");
icon("en_icons/down.png", "down");
icon("en_icons/rightup.png", "rightup");
icon("en_icons/rightdown.png", "rightdown");
icon("en_icons/conf_1.png", "conf_1");
icon("en_icons/conf_2.png", "conf_2");
icon("en_icons/right.png", "right");
icon("en_icons/subst.png", "subst");
icon("en_icons/loop_start.png", "loopstart");
icon("en_icons/loop_break.png", "loopbreak");
icon("en_icons/loop_end.png", "loopend");
icon("en_icons/math.png", "math");
icon("en_icons/funcf.png", "funcf");
icon("en_icons/funct.png", "funct");
icon("en_icons/funce.png", "funce");
...
function elmpr(num){
  ...
  plx[num] = -1;
  ply[num] = -1;
  alert("Could not put the icon in that location.\nInitialize the position.");
  return;
}
...
function optdlg(tar){
  ...
  if(opt[num[1]] != null && opt[num[1]] != 0 && skp){
    ...
    if(opv.length == 2){
      op1 = opv[1].split(';');
      dtl.insertAdjacentHTML('beforeend', '<h1 class="heading">[ ' + opv[0] + ' ] Setting completed</h1>\n');
      if(op1.length == 2){
        if(op1[0] == "p" || op1[0] == "l"){
    ...
    dtl.insertAdjacentHTML('beforeend', '<input type="button" value="Enter" onclick="dlgcls(' + "'dlg-detail', ''" + ')">\n');
    dtl.insertAdjacentHTML('beforeend', '<input type="button" value="Remove" onclick="elmrm(' + "'detail'" + ', ' + num[1] + ')">\n');
    dtl.showModal();
    return;
  }
  $("int-name").value = "";
  ...
  dlg.insertAdjacentHTML('beforeend', '<input type="button" id="optetr" value="Enter" onclick="optetr(' + "'" + typ[0] + "'" + ', ' + "'" + tag[obj[num[1]]] + "'" + ', ' + num[1] + ')">\n');
  dlg.insertAdjacentHTML('beforeend', '<input type="button" id="elmrm" value="Remove" onclick="elmrm(' + "'" + typ[0] + "'" + ', ' + num[1] + ')">\n');
  return;
}
...
function optetr(dlg, typ, num){
  ...
  if(typ == tag[0]){
    ...
    alert("An error has occurred. [Unexpected]");
    return;
  }
  return;
}
...
function elmrm(typ, num){
  ...
  if(id[0] == tag[22] || id[0] == tag[23]){
    alert("This icon cannot be erased.");
  }else{
...
function dlgerr(dlg, num, msg){
  if(msg == ""){
    alert('Icons' + num + ' : An error has occurred.');
  }else{
    alert('Icons' + num + ' : An error has occurred.\n\n' + msg);
  }
  dlg.close();
...
function exec(){
  ...
  for(lct = 1; run; lct++){
    if(lct == mxi){
      dlgerr(dlg, lct + ' (' + i + ', ' + j + ')', "The number of processes has reached the upper limit.\nIsn't it an infinite loop?");
      return;
    }
    if(plc[i][j] < 0){
      dlgerr(dlg, lct + ' (' + i + ', ' + j + ')', "The icons are discontinuous.");
      return;
    }else{
      typ = obj[plc[i][j]];
    }
    if(opt[plc[i][j]] == null){
      dlgerr(dlg, lct + ' (' + i + ', ' + j + ')', "The setting is not completed.");
      return;
    }
    if(typ == 0){
      dlgerr(dlg, lct + ' (' + i + ', ' + j + ')', "The start icon is in a mysterious position.");
      return;
    }else if(typ == 1){
      run = 0;
    }else if(typ == 2){
            ...
            if(ps3[0] == "p"){
              ...
              if(sub > 0){
                ...
                for(l = 0; l < tmp[k].length; l++){
                  ...
                  if(l == tmp[k].length){
                    dlgerr(dlg, lct + ' (' + i + ', ' + j + ')', "[" + prn[ps1[1]] + "] is not an available variable.");
                    return;
                  }
                }
                for(m = 0; m < tmp[k].length; m++){
                  ...
                  if(m == tmp[k].length){
                    dlgerr(dlg, lct + ' (' + i + ', ' + j + ')', "[" + prn[ps2[1]] + "] is not an available variable.");
                    return;
                  }
                }
                tmp[k][l][1] = tmp[k][m][1] / parseInt(ps3[1], 10);
      ...
      if(i > -1 && j > -1){  
        ...
        dlgerr(dlg, lct + ' (' + i + ', ' + j + ')', "Coordinate acquisition error.");
        return;
      }
    }else if(typ == 22){
      sub = sub + 1;
      ...
      if(i > -1 && j > -1){
        if(ret[2] > -1){
          if(prt[ret[2]] == tmp[ps1[1]][2]){
            ...
            dlgerr(dlg, lct + ' (' + i + ', ' + j + ')', "Substitution type error.");
            return;
          }
        }
        tmp.pop();
  ...
  dlg.insertAdjacentHTML('beforeend', '<input id="bpc" type="button" value="Convert to C" onclick="dcc(' + (pst[0] + 1) + ', ' + pst[1] + ', 0, 0)">\n');
  dlg.insertAdjacentHTML('beforeend', '<input id="kep" type="button" value="Save" onclick="keep()">\n');
  dlg.insertAdjacentHTML('beforeend', '<input id="bce" type="button" value="Close" onclick="dlgcls(' + "'dlg-exec', 'bpc:kep:bce'" + ')">\n');
  return;
}
...
function dcc(i, j, emd, ind){
  ...
  for(lct = 1; run; lct++){
    ...
    if(plc[i][j] < 0){
      dlgerr(dlg, lct, "The icons are discontinuous.");
      return;
    }else{
      typ = obj[plc[i][j]];
    }
    if(typ == 0){
      dlgerr(dlg, lct, "The start icon is in a mysterious position.");
      return;
    }else if(typ == 1){
      if(emd == 0){
        makeind(ccc, ind)
  ...
  dlg.insertAdjacentHTML('beforeend', '<input id="bcc" type="button" value="Close" onclick="dlgcls(' + "'dlg-ccc', 'bcc'" + ')">\n');
  dlg.showModal();
  return;
}
...
function init(){
  ...
  var describe = "en_icons/describe.png";
  var japanese = "en_icons/japanese.png";
  ...
  $("body").insertAdjacentHTML('beforeend', '<a href="' + location.href + '?lang=ja"><img style="position:absolute; left:' + ((pst[0] - 1) * ihw) + 'px; top:' + ((pst[1] - 1) * ihw) + 'px;" src="' + japanese + '" height="' + (ihw - 1) + 'px"></a>\n');
  ...
  if(typeof dsc.showModal === "function"){
    ...
    alert("This browser does not support modal.\nPlease change your browser.");
  }
  return;
}
